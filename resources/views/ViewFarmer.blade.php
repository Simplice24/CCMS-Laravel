...
<body>
  ...
  <div class="card-body">
    <div class="info">
    <p>Full name: {{$farmerinfo->name}}<p>
    <p>IDN: {{$farmerinfo->idn}}<p>
    <p>Cooperative name: {{$farmerinfo->cooperative_name}}<p>
    <p>Farmer category: {{$farmerinfo->category}}<p>
    <p>Gender: {{$farmerinfo->gender}}<p>
    <p>Number of trees: {{$farmerinfo->number_of_trees}}<p>
    <p>Fertilizer: {{$farmerinfo->fertilizer}}<p>
    <p>Phone: {{$farmerinfo->phone}}<p>
    <p>Province: {{$farmerinfo->province}}<p>
    <p>District: {{$farmerinfo->district}}<p>
    <p>Sector: {{$farmerinfo->sector}}<p>
    <p>Cell: {{$farmerinfo->cell}}<p>
    <a href="<?=url('viewfarmers');?>"><button type="button" class="btn btn-primary">Go back</button></a>
  </div>
      </div>
    </div>
